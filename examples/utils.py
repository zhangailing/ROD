...
def munkres(pred_label, true_label):
...
def get_roc_score_ensemble(z_list, adj_orig, edges_pos, edges_neg):
    def sigmoid(x): 
        return 1 / (1 + np.exp(-x))
    ...
    for i in range(len(z_list)):
        adj_rec_list.append(np.dot(z_list[i], z_list[i].T))
    adj_rec = adj_rec_list[0]
    for _ in range(1, len(z_list)):
        adj_rec += adj_rec_list[i]
    adj_rec /= len(z_list)
